    ...
    public void paymentCallback(@RequestParam Map<String, String> queryParams, HttpServletResponse response, Authentication authentication) throws IOException, ParseException {
        ...
        if((billId!= null && !billId.equals(""))||(registerServiceId!= null && !registerServiceId.equals(""))) {
            if ("00".equals(vnp_ResponseCode)) {
                ...
                BillEntity bill = billService.findById(Long.valueOf(queryParams.get("billId"))).orElse(null);
//                if (bill==null){
...
//                }
                ...
                if (bill.getStatus().equals(-2)&&bill.getPaymentType().equals(1)){
                    bill.setStatus(7);
                }else if (bill.getStatus().equals(-2)&&bill.getPaymentType().equals(2)){
                    bill.setStatus(4);
                }
    ...
    public ResponseEntity getPay(@PathVariable Long billID, @RequestParam(required = false) String admin) throws UnsupportedEncodingException {
        ...
        long millisecondsDifference = currentTimestamp.getTime() - bill.getCreateDate().getTime();
